...
try {
    ...
    foreach ($Required in @(
        ...
        'media.v1', 'source.v1', 'properties.v1'
    )) {
    ...
    $CapabilitiesQuery = Send-V2Request @{ op = 'request'; id = 'task11.capabilities'; method = 'engine.getCapabilities'; params = @{} }
    Assert-Ok $CapabilitiesQuery 0 'engine.getCapabilities'
    if (@($CapabilitiesQuery.data.capabilities | Where-Object { [string]$_.name -eq 'filter.v1' }).Count -ne 1) {
        Fail 'engine.getCapabilities did not advertise filter.v1 exactly once.'
    }
    ...
    if ([int64]$Defaults.data.settings.value -ne 0) {
        Fail 'filter.kindDefaults returned the wrong deterministic value.'
    }
    ...
    if ([int]$Empty.data.count -ne 0) { Fail 'new parent unexpectedly contained filters.' }
    ...
    $GuardedRead = Send-V2Request @{ op = 'request'; id = 'task11.guardedRead'; method = 'filter.get'; ifRevision = 2; params = @{ filter = '2' } }
    Assert-Error $GuardedRead 'bad_request' 2 'ifRevision on filter.get'
    ...
    if ([string]$FilterProperties.data.target.type -ne 'filter' -or [int64]$FilterProperties.data.settings.value -ne 10) {
    ...
    if ([int64]$Patch.data.settings.value -ne 20) { Fail 'patch did not settle at value 20.' }
    ...
    if ([int64]$Replace.data.settings.value -ne 30) { Fail 'replace did not settle at value 30.' }
    ...
    $DisableNoop = Send-V2Request @{
        op = 'request'; id = 'task11.disableNoop'; method = 'filter.setEnabled'; ifRevision = 5
        params = @{ filter = '2'; enabled = $false }
    }
    Assert-Ok $DisableNoop 5 'idempotent filter.setEnabled(false)'
    if ($script:Events.Count -ne 0) { Fail 'idempotent filter.setEnabled unexpectedly queued an event.' }
    ...
    if ($Enabled.data.enabled) { Fail 'filter remained enabled after disable.' }
    ...
    $Create3 = Send-V2Request @{
        ...
        params = @{ source = '1'; kind = 'task11_filter'; name = 'second-filter'; settings = @{ value = 3 } }
    }
    ...
    $Create4 = Send-V2Request @{
        ...
        params = @{ source = '1'; kind = 'task11_filter'; name = 'third-filter'; settings = @{ value = 4 } }
    }
    ...
    $SetOrderNoop = Send-V2Request @{
        op = 'request'; id = 'task11.setOrderNoop'; method = 'filter.setOrder'; ifRevision = 9
        params = @{ filter = '2'; index = 0 }
    }
    Assert-Ok $SetOrderNoop 9 'idempotent filter.setOrder'
