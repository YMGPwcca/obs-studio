...
private:
	...
	void release_item(std::unordered_map<uint64_t, ItemEntry>::iterator &it)
	{
...
int main(int argc, char **argv)
{
	...
	try {
		...
		for (;;) {
			...
			ObsDataPtr request(obs_data_create_from_json(line.c_str()));
			if (!request) {
			...
			try {
				...
				send_error(0, "internal_error", "command failed internally");
			} catch (...) {
