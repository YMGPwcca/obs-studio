...
bool settle_deferred_filter_update(FilterV2State &state, uint64_t handle, const std::string &expected_settings,
					   RuntimeV2Result &result)
{
	...
	// Close the race between the initial deferred scan and reading the permanent
	// observer generation. The observer publishes the normalized batch before it
	// advances update_generation.
	if (promote_deferred_filter_update(state, handle, expected_settings, result))
		return true;
...
void Engine::v2_settle_filter_mutation(obs_data_t *params, RuntimeV2Result &result)
{
	...
	if (deferred_video &&
	    (expected.empty() || !settle_deferred_filter_update(*filter_v2_state_, handle, expected, result))) {
		std::fprintf(stderr,
