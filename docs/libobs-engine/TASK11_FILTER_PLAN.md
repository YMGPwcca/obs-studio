...
- `source.duplicate` copies libobs filters; copied filters receive fresh handles
  and command-owned `filter.created` events in the source-duplicate revision.
...
- source duplication exposes copied filters with `filter.created` in the same
  revision;
